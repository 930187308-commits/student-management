...
本轮主工作台第一优先和第二优先优化已完成，详见：

- `docs/WORKBENCH_OPTIMIZATION_REPORT_2026-06-22.md`

当前重点结论：

- 学员详情页已经收口为：顶部摘要、基础信息、入班记录、折叠沟通、展开成绩。
- 学员详情页新增最近时间线，聚合收费、成绩、沟通、考勤和操作记录。
- 首页今日工作台已保留经营复盘、待续费/欠费、意向跟进、AI 对话四个高频入口。
- 首页快捷按钮改为紧凑网格，待办为空时显示推荐事项。
- 顶部新增操作日志入口，核心业务新增、编辑、删除会自动记录。
- AI 系统问答固定回归测试集已维护，当前 `27 / 27` 通过。
- 修复“本月课消情况怎么样？”误触发经营复盘的问题。
- 资料库定位保持为 AI 可引用资料中心，题库细节继续由独立题库工作台承载。
...
### 9A 数学题库系统输入输出闭环（2026-06-08）

**目标**：参考题库工作流，把知识库中的题库模块从“基础录入”扩展为可日常使用的题目收集、分类、筛选、选题和输出系统。

#### A. 后端题库字段
- `question_items` 新增特殊格式与组卷字段：
  - `formula_latex`
  - `diagram_svg`
  - `image_url`
  - `estimated_minutes`
  ...
  - `origin_text`
  - `ai_notes`
- `knowledge-service` 支持这些字段读写，并兼容旧数据库自动补列。
...
- 题库页面改为三栏工作区：
  ...
  - 右侧：输出生成
- 支持关键词、年级、章节、难度、题型筛选。
- 支持勾选题目、选择当前可见题目、清空选题篮。

#### C. 特殊格式处理
- 题目可保存 LaTeX 公式。
- 几何草图可保存 SVG，并在网页端展示。
- 复杂图片题可保存图片链接或本地路径。
- 保留原始输入/OCR 文本，方便后续复核。

#### D. AI 辅助录入
- 新增“AI 辅助录入”入口。
- 当前先用本地规则把原始题目文本预填为结构化字段：
  - 年级
  - 章节
  - 题型
  - 难度
...
- 支持生成：
  ...
  - 专题讲义
  - 举一反三练习
- 输出包含分值、建议用时、答案、解析、易错提醒、公式/图形提示。
- 支持复制和下载 Markdown。
...
#### G. 文档
- 新增 `docs/STAGE_9_QUESTION_BANK_SYSTEM.md`，记录题库系统方案、已实现功能、图形公式处理方式和后续维护流程。
...
- `js/knowledge.js`
- `PROJECT_PROGRESS.md`
- `docs/STAGE_9_QUESTION_BANK_SYSTEM.md`

### 10A 独立数学题库工作台（2026-06-08）

**目标**：用户试用后确认 Stage 9 题库仍像“题目陈列”，不足以作为成熟题库系统使用。因此参考成熟教培题库/组卷产品，新增独立题库工作台，不再把题库强行塞进知识库小 tab。
...
- 成熟题库系统重点在完整流程：
  ...
  - 错题/学情回流
- 参考方向：
  - 菁优网：多知识点筛题、平行组卷、反向细目表、AI 分析/改编
  - 组卷网：在线选题、组卷、布置作业、学情分析
  - 橡皮网：章节出题、知识点出题、智能出题、细目表出题、答题卡
  - AI 题库工具：公式/图形识别、AI 生成练习、在线编辑打印

#### B. 新增独立页面
- 新增 `question-bank.html`
- 新增 `js/question-bank-system.js`
- 主导航“AI 与知识”下新增“数学题库”入口
- 知识库页新增“打开独立数学题库工作台”按钮
- 后端静态白名单允许访问 `question-bank.html`
...
- 精准筛选：
  ...
  - 含公式/图形/解析/易错点
- 题目列表：
  - 显示年级、章节、题型、难度、分值、预计时间
  - 显示公式、SVG 图形、图片路径、答案、知识点
  - 支持加入/移出题篮
  - 支持编辑题目
- 题篮：
  - 题数、总分、预计分钟
...
- AI 辅助录入：
  - OCR/手打题目粘贴
  - 本地规则预填字段
  - 调用真实 AI `question-classify` 生成分类、易错点、讲法建议
  - 保存前进入编辑表单人工确认

#### D. 验证
- `npm run check` 通过
- 浏览器验证：
  - `/question-bank.html` 可打开
  - 页面加载现有 5 道示例题
  - “几何专题”筛选得到 1 题
  - 筛选结果加入题篮可生成测试卷
  - 输出总分 37 分、含图形提示
  - AI 辅助录入可预填年级、章节、答案、解析和 AI 说明
