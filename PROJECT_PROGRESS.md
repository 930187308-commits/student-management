...
- 第二批真实数据录入优化，由 MacBook / Claude Code 先实现，Mac mini / Codex 验收合并：
  - 意向学员模板补充年级、微信号
  - 学校分布统计
  - 课消统计口径统一
  - 首页班级概览增加总课时、已消课时
  - 学员状态增加“组班中”，支持加入组班中班级
- 继续记录真实录入过程中发现的细节问题。
- 后续再规划从整包 `/data` 逐步拆分到模块 API。

## 阶段 3：前端逐步接 API

状态：未开始，放在真实数据录入体验稳定之后
